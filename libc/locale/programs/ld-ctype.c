...
#define CTYPE_UINT32(name, value)					      \
	  ...
	  case _NL_ITEM_INDEX (_NL_CTYPE_CLASS_NAMES):
	    ...
	    /* ??? We used to insert 4 bytes of alignment if the output
	       was already four-byte aligned at this point.  It's unclear
	       how deliberate that was, but it's preserved here for
	       consistency.  */
	    add_locale_char (&file, 0);
	  ...
	  case _NL_ITEM_INDEX (_NL_CTYPE_MAP_NAMES):
	    ...
	    add_locale_char (&file, 0);
	    /* ??? See _NL_CTYPE_CLASS_NAMES comment above.  */
	    add_locale_char (&file, 0);
	    align_locale_data (&file, 4);
	    end_locale_structure (&file);
	    break;
